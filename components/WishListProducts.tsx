...
const WishListProducts = () => {
  ...
  const handleResetWishlist = () => {
    const confirmReset = window.confirm(
      "Apa kamu serius untuk mereset barang Favorite ini?"
    );
    if (confirmReset) {
      resetFavorite();
      toast.success("Favorite sukses diReset");
    }
  };
            ...
            <table className="w-full border-collapse">
              <thead className="border-b">
                <tr className="bg-black/5">
                  ...
                  <th className="p-2 text-left hidden md:table-cell">Tipe</th>
                  <th className="p-2 text-left hidden md:table-cell">Status</th>
                  <th className="p-2 text-left">Harga</th>
                  <th className="p-2 text-center md:text-left">Tambahkan</th>
                </tr>
              </thead>
              <tbody>
                {favoriteProduct
                      ...
                      <td className="px-2 py-4 flex items-center gap-2">
                        <X
                          onClick={() => {
                            ...
                            toast.success("Prodak dihapus dari daftar Favorite");
                          }}
                          size={18}
        ...
        <div className="flex min-h-[400px] flex-col items-center justify-center space-y-6 px-4 text-center">
          ...
          <div className="space-y-2">
            <h2 className="text-2xl font-semibold tracking-tight">
              Tidak ada Barang Favoritmu!
            </h2>
            <p className="text-sm text-muted-foreground">
              Item yang ditambahkan ke daftar Favorite Anda akan muncul di sini.
            </p>
          </div>
          <Button asChild>
            <Link href="/shop" className="text-shop_darkColor/95 bg-shop_btn_dark_green hover:bg-shop_dark_green hover:text-shop_light_bg">Lanjut belanja</Link>
          </Button>
        </div>
      )}
    </Container>
